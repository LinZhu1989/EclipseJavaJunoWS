...
public class HelperUI extends JFrame implements ActionListener {
	...
	private MyLabel lbl_title, lbl_blank, lbl_name, lbl_pin, lbl_number,
			lbl_nameInfo, lbl_pinInfo, lbl_numberInfo, lbl_serverInfo,
			lbl_browserInfo, lbl_selectInfo;
	...
	private Color color_title, color_label, color_label2, color_warning,
			color_disable, color_txt;
	private Dimension size_default, size_txt, size_wider, size_XWider,
			size_small;
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (ae.getSource() == btn_browser) {
			...
			if (result == 0) {
				try {
					...
					lbl_browserInfo
							.setText("<HTML>The path of your browser you selected is:"
									+ browserLocation
									+ "<br>You use browser:"
		...
		if (ae.getSource() == btn_reset) {
			...
			lbl_nameInfo
					.setText("<HTML>Please type the email address you registerd for your LinkedIn Account.</html>");
			lbl_pinInfo
					.setText("<HTML>Please type the password of your account. <br>Your password will be used but not saved.</html>>");
			lbl_selectInfo
					.setText("<HTML>Please select what you want to do, adding connections or endorsing.</html>");
			lbl_numberInfo
					.setText("<HTML>Please type the totol number of actions you want, either for adding connections or endorsing.</html>");
			lbl_serverInfo
					.setText("<HTML>Please run the selenium server by locating the path of file selenium-server-standalone-2.43.1.jar. Make sure all the files (including the selenium****.jar and linkedInHelper.jar)are in the same level.</html>");
			lbl_browserInfo
					.setText("<HTML><HTML>Please find your Browser.exe file to lacate the path of your browser.(eg. firefox.exe) <br>Now only firefox browser is supported. Chrome support is coming soon!");
			ifCanRun = false;
	...
	private void runApp() {
		...
		LinkedInHelper linkedin = new LinkedInHelper(txtName.getText(),
				new String(txtPIN.getPassword()), "https://www.linkedin.com",
				browser.toString().toLowerCase(), browserLocation, 4444,
	...
	private boolean checkValues() {
		...
		if (txtNum.getText() == null) {
			...
			try {
				...
				if (number < 0) {
					lbl_numberInfo
							.setText("Please type a positive total number!");
					showWarningColor(lbl_numberInfo, true);
					result = false;
				} else {
					lbl_numberInfo
							.setText("<HTML>[OK!] The total number you enter is "
									+ number + "</HTML>");
		...
		if (userChoice == UserSelection.DEFAULT) {
			...
			lbl_selectInfo.setText("<HTML>[OK!] You have selected "
					+ userChoice + ".</HTML>");
		...
		if (browserLocation == null) {
			lbl_browserInfo
					.setText("Sorry, you did not locate the browser file.");
			...
			lbl_browserInfo
					.setText("Only Firefox Browser is supported, sorry.");
		...
		if (!ifServerRun) {
			lbl_serverInfo
					.setText("Sorry, you have not run the selenium server!");
			showWarningColor(lbl_serverInfo, true);
			result = false;
		} else {
			lbl_serverInfo
					.setText("<HTML>[OK!] The server has been run! Please run it again if you close the server manually. Because shut-down operation will not be detected.</HTML>");
