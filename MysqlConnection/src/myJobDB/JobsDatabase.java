...
public class JobsDatabase {
	...
	public static void insertNewJobs(JobDatabaseManager jobDBM,String[] info) {
		...
		int id = Integer.valueOf(jobDBM.SelectSql("Select * from linzhu1989.jobs order by id desc limit 0,1").get(0)[0]);
	...
	public static void updateJobStatus(JobDatabaseManager jobDBM,String info,String ID) {
		
		jobDBM.logger.appendWrite("Prepare to update Status of Job "+ID+" to "+info);
		jobDBM.UpdateSql("UPDATE `linzhu1989`.`jobs` SET `status`='"+info+"' WHERE `id`='"+ID+"'");
